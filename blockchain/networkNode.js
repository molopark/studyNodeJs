...
app.post('/register-and-broadcast-node',function(req,res){
  ...
  bitcoin.networkNodes.forEach(networkNodes => {
    console.log('networkNodes:'+JSON.stringify(networkNodes));
    //register - node
    const requestOption = {
    ...
    console.log('requestOption:'+JSON.stringify(requestOption));
    //순차적으로 비동기를 실행하기 위해서 배열에 넣음
    regNodesPromises.push(rp(requestOption))
  }); //for 문 끝
  console.log('regNodesPromises:'+JSON.stringify(regNodesPromises));
...
app.post('/register-node',function(req,res){
  ...
  //새로운 노드 주소
  const newNodeUrl = req.body.newNodeUrl;
  console.log('newNodeUrl:'+JSON.stringify(newNodeUrl));
